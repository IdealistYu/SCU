...
function loadContent() {
    const filePath = isChinese ? 'resource/section_2_chinese.txt' : 'resource/section_2_english.txt';
    fetch(filePath)
...
function playAudio(language) {
    const audioFilePath = language === 'chinese' ? 'resource/第二单元 单元说明.m4a' : 'resource/第二单元单元说明.mp3';
...
function backgroundMusic() {
    const bgMusic = new Audio('resource/凝安·黄钟为宫.mp3');
    bgMusic.loop = true;
    bgMusic.play();
}
