...
function loadContent() {
    const filePath = isChinese ? 'resource/section_1_chinese.txt' : 'resource/section_1_english.txt';
    fetch(filePath)
...
function playAudio(language) {
    const audioFilePath = language === 'chinese' ? 'resource/第一单元 单元说明.m4a' : 'resource/第一单元单元说明.mp3';
...
function backgroundMusic() {
    const bgMusic = new Audio('resource/【原钟孤版录音】曾侯乙编钟之声.mp3');
    bgMusic.loop = true;
    bgMusic.play();
}
